...
def visualize_path(domain, paths, pred_traj, dom_size, correct):
    fig, ax = plt.subplots()
    ax.imshow(domain.T, cmap="Greys_r")
    ...
    fig.savefig(
        '../image/path/path{}_{}.pdf'.format(int(dom_size), int(correct)))
    plt.close()


def visualize_reward(reward_image, dom_size, correct):
    fig, ax = plt.subplots()
    reward_map = ax.imshow(reward_image.T, cmap=cm.cool)
    fig.colorbar(reward_map, ax=ax)
    fig.savefig(
        '../image/reward/reward{}_{}.pdf'.format(int(dom_size), int(correct)))
    plt.close()


def visualize_v_value(v_value_image, dom_size, correct):
    fourcc = cv2.VideoWriter_fourcc('m', 'p', '4', 'v')
    video = cv2.VideoWriter('../video/v_value{}_{}.mp4'.format(
        int(dom_size), int(correct)), fourcc, 20.0, (560, 480))
    ...
    for i in range(v_value_image.shape[0]):
        ...
        fig.savefig(
            '../image/v_value/v_value{}_{}_{}.png'.format(int(dom_size), int(correct), i))
        plt.close()
