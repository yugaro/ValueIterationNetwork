...
class GraphBase(object):
    ...
    def set_graph(self, image):
        # Set node
        for x in range(self.domsize[0]):
            for y in range(self.domsize[1]):
                if image[y, x] == 0:
                    self.add_node((x, y))

        # Set edge
        for x1 in range(self.domsize[0]):
            for y1 in range(self.domsize[1]):
                for dx in range(-1, 2):
                    for dy in range(-1, 2):
                        x2 = x1 + dx
                        y2 = y1 + dy
                        if x2 < 0 or x2 >= self.domsize[0]:
                            continue
                        if y2 < 0 or y2 >= self.domsize[1]:
                            continue
                        if image[y1, x1] == 0 and image[y2, x2] == 0:
                            self.add_halfedge(
                                (x1, y1), (x2, y2), math.sqrt(dx * dx + dy * dy))
