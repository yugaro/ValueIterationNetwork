...
def main():
    ...
    while True:
        ...
        test_img = cv2.cvtColor(im, cv2.COLOR_GRAY2BGR)
        reward = (255 * (reward - np.min(reward)) /
                  (np.max(reward) - np.min(reward))).astype(np.uint8)
        value = (255 * (value - np.min(value)) /
                 (np.max(value) - np.min(value))).astype(np.uint8)
        ...
        cv2.imshow("image", cv2.resize(255 - test_img * 255,
                                       (300, 300), interpolation=cv2.INTER_NEAREST))
        cv2.imshow("reward", cv2.resize(
            reward, (300, 300), interpolation=cv2.INTER_NEAREST))
        cv2.imshow("value", cv2.resize(
            value, (300, 300), interpolation=cv2.INTER_NEAREST))
        cv2.waitKey(0)
