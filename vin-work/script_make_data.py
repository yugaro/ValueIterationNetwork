...
def sample_graph_traj(graph, num_trajs):
    ...
    while traj < num_trajs:
        ...
        if trial > num_trajs * 2:
            break
        pos = (random.randint(1, domsize[0] - 1),
               random.randint(1, domsize[1] - 1))
...
def main():
    ...
    max_obs_size = 1.0
    num_trajs = 7
    maxSamples = int(num_domains * num_trajs * max_traj_len / 2)
    ...
    while dom <= num_domains:
        ...
        for i in range(len(states_xy)):
            if len(states_xy[i]) > 0:
                ...
                state_xy_data[num_samples:num_samples +
                              ns] = np.array(states_xy[i][0:-1])
